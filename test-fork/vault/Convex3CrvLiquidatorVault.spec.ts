...
describe("Convex 3Crv Liquidator Vault", async () => {
    ...
    describe("withdraw should round up", async () => {
        ...
        it("withdrawing assets should round up", async () => {
            ...
            await musdConvexVault.connect(staker1.signer)["deposit(uint256,address)"](10, staker1.address)
            await threeCrvToken.connect(staker1.signer).transfer(musdConvexVault.address, 1)

            const userSharesBefore = await musdConvexVault.balanceOf(staker1.address)
            ...
            await musdConvexVault.connect(staker1.signer).withdraw(3, staker1.address, staker1.address)
            const userSharesAfter = await musdConvexVault.balanceOf(staker1.address)
