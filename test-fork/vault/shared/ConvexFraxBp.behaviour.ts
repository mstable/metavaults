...
export interface ConvexFraxBpContext {
    vault: ConvexFraxBpVault
    owner: Account
...
export const behaveLikeConvexFraxBpVault = (ctx: () => ConvexFraxBpContext): void => {
    ...
    describe("EIP-4626 view functions", () => {
        it("asset()", async () => {
            ...
            expect(await vault.asset(), "asset").eq(crvFRAX.address)
    ...
    describe("EIP-4626 operations", () => {
        ...
        it("user deposits crvFrax assets to vault", async () => {
            ...
            expect(ownerAssetsBefore, "crvFrax bal >= deposit amount").to.gte(amounts.initialDeposit)
        ...
        it("user redeems some shares from vault", async () => {
            ...
            // Assets
            const assetsWithdrawn = await crvFraxToken.balanceOf(receiver)
            expect(assetsWithdrawn, "withdrawn assets == preview assets").eq(previewAssets)
            expect(await crvFraxToken.balanceOf(receiver), "receiver assets").eq(assetsWithdrawn)
            // Shares
        ...
        it("user withdraws some crvFrax assets from vault", async () => {
            const { amounts, crvFraxToken, owner, vault } = ctx()
            ...
            // Assets
            expect(await crvFraxToken.balanceOf(receiver), "withdrawn assets == actual assets").eq(amounts.withdraw)
            // Shares
        ...
        it("user mints shares from vault", async () => {
            ...
            // Assets
            const ownerAssetsAfter = await crvFraxToken.balanceOf(owner.address)
        ...
        it("user deposits crvFrax assets to vault with custom slippage", async () => {
            const { amounts, crvFraxToken, owner, vault } = ctx()
            ...
            expect(await crvFraxToken.balanceOf(owner.address), "crvFrax bal >= deposit amount").to.gte(amounts.initialDeposit)
        ...
        it("user redeems all shares", async () => {
            const { crvFraxToken, owner, vault } = ctx()
