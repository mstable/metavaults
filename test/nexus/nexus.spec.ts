import { KEY_LIQUIDATOR, ONE_DAY, ONE_WEEK, ZERO, ZERO_ADDRESS, ZERO_KEY } from "@utils/constants"
    ...
    describe("initialize()", () => {
        ...
        context("should succeed", () => {
            it("with default modules", async () => {
                await nexus.initialize([KEY_LIQUIDATOR], [sa.liquidator.address], [false], sa.governor.address)
                // initialized
                const initialized = await nexus.initialized()
                expect(initialized).to.equal(true)

                // validate modules
                await expectInModules(nexus, "Liquidator", sa.liquidator.address, false)
            })
