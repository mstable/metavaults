...
const calculateAssetsWithdraw = (swaps:Array<Swap>, vaultIndex:number)=>swaps.filter(s => s.fromVaultIndex === vaultIndex).map(s=>s.assets).reduce(sumBN,BN.from(0))
const calculateAssetsDeposit = (swaps:Array<Swap>, vaultIndex:number)=>swaps.filter(s => s.toVaultIndex === vaultIndex).map(s=>s.assets).reduce(sumBN,BN.from(0))
// 
const calculateSharesRedeem = (swaps:Array<Swap>, vaultIndex:number)=>swaps.filter(s => s.fromVaultIndex=== vaultIndex).map(s=>s.shares).reduce(sumBN,BN.from(0))
const calculateSharesRedeemTo = (swaps:Array<Swap>, vaultIndex:number)=>swaps.filter(s => s.toVaultIndex === vaultIndex).map(s=>s.shares).reduce(sumBN,BN.from(0))
...
const calculateVaultDataSwap = async (ctx: SameAssetUnderlyingsAbstractVaultBehaviourContext,swaps:Array<Swap>, vaultIndex:number ,bVaultDataBefore:BasicVaultData)=>{
    const { vault, sa } = ctx;
    ...
    const vaultShares = bVaultDataBefore.vaultShares.add((await bVault.convertToShares(vaultSharesDelta.abs())).mul(vaultSharesDelta.isNegative()?-1:1))
    
    return {totalAssets, totalSupply, vaultShares}
}

async function expectRebalance(
    ...
    assertBNClosePercent( dataAfter.bVault0Data.totalAssets, expectedVault0Data.totalAssets, variances.rebalancebVault0,"underlying vault 0 totalAssets")    
    assertBNClosePercent( dataAfter.bVault0Data.totalSupply, expectedVault0Data.totalSupply, variances.rebalancebVault0,"underlying vault 0 totalSupply")    
    assertBNClosePercent( dataAfter.bVault0Data.vaultShares, expectedVault0Data.vaultShares, variances.rebalancebVault0,"underlying vault 0 balanceOf")    

    // underlying vault 2 , state changes after re-balance
    assertBNClosePercent( dataAfter.bVault1Data.totalAssets, expectedVault1Data.totalAssets, variances.rebalancebVault1,"underlying vault 1 totalAssets")    
    assertBNClosePercent( dataAfter.bVault1Data.totalSupply, expectedVault1Data.totalSupply, variances.rebalancebVault1,"underlying vault 1 totalSupply")    
    assertBNClosePercent( dataAfter.bVault1Data.vaultShares, expectedVault1Data.vaultShares, variances.rebalancebVault1,"underlying vault 1 balanceOf")    
        ...
        describe("rebalance", async () => {
            ...
            context("using shares", async () => {
                it("100% from vault0 to vault1", async () => {
                    ...
                    const swap = { fromVaultIndex: 0, toVaultIndex: 1, 
                        assets:  BN.from(0), 
                ...
                it("50% from vault1 to vault0", async () => {
                    ...
                    const swap = { fromVaultIndex: 1, toVaultIndex: 0, 
                        assets:  BN.from(0), 
        ...
        describe("add vault", async () => {
            ...
            context("success", async () => {
                ...
                before(async () => {
                    ...
                    console.log("🚀 ~ file: SameAssetUnderlyingsAbstractVault.behaviour.ts ~ line 360 ~ before ~ bVaultNewIndex", bVaultNewIndex)
