...
contract Nexus is INexus, DelayedClaimableGovernor {
    ...
    struct Proposal {
        ...
        uint32 timestamp; // Timestamp when module upgrade was proposed
    }
    ...
    ****************************************/
    ...
    function proposeModule(bytes32 _key, address _addr) external override onlyGovernor {
        ...
        Module memory module = modules[_key];
        require(module.addr != _addr, "Module already has same address");
        require(!module.isLocked, "Module must be unlocked");

        require(proposedModules[_key].timestamp == 0, "Module already proposed");
        proposedModules[_key] = Proposal(_addr, SafeCast.toUint32(block.timestamp));
    ...
    function _publishModule(
        ...
        modules[_key] = Module(_addr, _isLocked);
